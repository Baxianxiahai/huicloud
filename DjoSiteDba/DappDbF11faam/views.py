...
class dct_classDbiL3apF11Faam:
    ...
    def dft_dbi_factory_table_modify(self, inputData):
        ...
        dct_t_l3f11faam_factory_sheet.objects.filter(sid=sid).update(pjcode=pjCode, workstart=workStart,
                                                                     workend=workEnd, reststart=restStart,
                                                                     restend=restEnd, fullwork=fullWork,
                                                                     address=address, latitude=latitude,
                                                                     longitude=longitude, trafficmoney=trafficmoney,
                                                                     factorybonus=factorybonus, memo=memo)
    ...
    def dft_dbi_factory_table_new(self, inputData):
        ...
        dct_t_l3f11faam_factory_sheet.objects.create(pjcode=pjCode, workstart=workStart, workend=workEnd,
                                                     reststart=restStart, restend=restEnd, fullwork=fullWork,
                                                     address=address, latitude=latitude, longitude=longitude,
                                                     trafficmoney=trafficmoney, factorybonus=factorybonus, memo=memo)
    ...
    def dft_dbi_product_type_modify(self, inputData):
        ...
        dct_t_l3f11faam_type_sheet.objects.filter(sid=sid).update(typecode=typeCode, applenum=appleNum,
                                                                  appleweight=appleWeight, applegrade=appleGrade,
                                                                  memo=memo)
    ...
    def dft_dbi_product_type_new(self, inputData):
        ...
        if appleNum == "": appleNum = 0
        if appleWeight == "": appleWeight = 0
        dct_t_l3f11faam_type_sheet.objects.create(typecode=typeCode, applenum=appleNum, appleweight=appleWeight,
                                                  applegrade=appleGrade, memo=memo)
    ...
    def dft_dbi_staff_table_new(self, inputData):
        ...
        if (os.path.exists(file_link)):
            ...
            dct_t_l3f11faam_member_sheet.objects.create(mid=mid, pjcode=pjCode, employee=employee, gender=gender,
                                                        ...
                                                        standardnum=standardNum, memo=memo)
        else:
            dct_t_l3f11faam_member_sheet.objects.create(mid=mid, pjcode=pjCode, employee=employee, gender=gender,
                                                        phone=phone, regdate=now, position=position, zone=zone,
                                                        idcard=idCard, address=address, bank=bank, bankcard=account,
                                                        unitprice=unitPrice, standardnum=standardNum, memo=memo)
        return True
    ...
    def dft_dbi_staff_table_modify(self, inputData):
        ...
        if (os.path.exists(file_link)):
            ...
            if os.path.exists(file_link_new):
                os.unlink(file_link_new)
            os.rename(file_link, file_link_new)
            dct_t_l3f11faam_member_sheet.objects.filter(mid=staffId).update(pjcode=pjCode, employee=employee,
                                                                            openid=nickname, gender=gender,
                                                                            phone=phone, regdate=now, position=position,
                                                                            zone=zone,
                                                                            idcard=idCard, address=address, bank=bank,
                                                                            bankcard=account,
                                                                            photo=file_new_name, unitprice=unitPrice,
                                                                            standardnum=standardNum, onjob=onjob,
                                                                            memo=memo)
        else:
            dct_t_l3f11faam_member_sheet.objects.filter(mid=staffId).update(pjcode=pjCode, employee=employee,
                                                                            openid=nickname, gender=gender,
    ...
    def dft_dbi_staff_table_query(self, inputData):
        ...
        if (containLeave == 'true'):
            if keyword == "":
                ...
                for line in result:
                    if line.onjob == True:
                        ...
                        onJob = 0
                    temp = {'id': str(line.mid), 'name': str(line.employee), 'PJcode': str(line.pjcode),
                            'nickname': str(line.openid),
                            'mobile': str(line.phone), 'gender': str(line.gender), 'identify': str(line.idcard),
                            'geoinfo': str(line.zone),
                            'address': str(line.address), 'bank': str(line.bank), 'account': str(line.bankcard),
                            'photo': str(self.__MFUN_HCU_FAAM_EMPLOYEE_PHOTO_WWW_DIR + str(line.photo)),
                            'salary': str(line.unitprice), 'position': str(line.position), 'status': str(onJob),
                            'KPI': str(line.standardnum),
                            'memo': str(line.memo)}
                    staffTable.append(temp)
            else:
                result = dct_t_l3f11faam_member_sheet.objects.filter(Q(pjcode=pjCode),
                                                                     Q(employee__icontains=keyword) | Q(
            ...
            if keyword == "":
                ...
                result = dct_t_l3f11faam_member_sheet.objects.filter(Q(pjcode=pjCode), Q(onjob=onJob),
                                                                     Q(employee__icontains=keyword) | Q(
                                                                         phone__icontains=keyword))
                for line in result:
                    if line.onjob == True:
    ...
    def dft_dbi_attendance_record_new(self, inputData):
        ...
        if (dateNow != workDay):
            ...
            if factory_config == "":
                ...
                if workTime < 0:
                    workTime = 0
                if arriveTimeInt <= stdWorkStart:
                    laterWorkFlag = False
                else:
                    laterWorkFlag = True
                if leaveTimeInt >= stdWorkEnd:
                ...
                if result.exists():
                    ...
                    result = dct_t_l3f11faam_daily_sheet.objects.filter(employee=employee, pjcode=pjCode,
                                                                        workday=workDay)
                    if result.exists():
                        dct_t_l3f11faam_daily_sheet.objects.filter(employee=employee, pjcode=pjCode,
                                                                   workday=workDay).update(arrivetime=arriveTime,
                                                                                           leavetime=leaveTime,
                                                                                           offwork=offWorkTime,
                                                                                           unitprice=unitPrice,
                                                                                           laterflag=laterWorkFlag,
                                                                                           earlyflag=earlyLeaveFlag)
                    else:
                        ...
                        dct_t_l3f11faam_daily_sheet.objects.create(pjcode=pjCode, daystandardnum=dayStandNumber,
                                                                   employee=employee, workday=workDay,
                                                                   arrivetime=arriveTime, leavetime=leaveTime,
                                                                   offwork=offWorkTime, worktime=workTime,
                                                                   unitprice=unitPrice, laterflag=laterWorkFlag,
                                                                   earlyflag=earlyLeaveFlag)
                else:
                    return False
            ...
            if factory_config == "":
                ...
                if result.exists():
                    ...
                    result = dct_t_l3f11faam_daily_sheet.objects.filter(employee=employee, pjcode=pjCode,
                                                                        workday=workDay)
                    if result.exists():
                        dct_t_l3f11faam_daily_sheet.objects.filter(sid=sid).update(workday=workDay,
    ...
    def dft_dbi_production_history_audit(self, inputData):
        ...
        if keyWord == "":
            ...
            result = dct_t_l3f11faam_production.objects.filter(Q(pjcode=pjCode), Q(owner__icontains=keyWord) | Q(
                typecode__icontains=keyWord), Q(activetime__range=(dayTimeStart, dayTimeEnd)))
            for line in result:
                #                 activeTime=line.activetime
                #                 if activeTime==None:
                #                     activeTime="1900-01-01 00:00:00"
    ...
    def dft_dbi_employee_kpi_audit(self, inputData):
        ...
        if keyWord != "":
            result = dct_t_l3f11faam_daily_sheet.objects.filter(pjcode=pjCode, employee__icontains=keyWord,
                                                                workday__range=(dayTimeStart, dayTimeEnd))
            for line in result:
        ...
        productBuf = []
        if keyWord != "":
            result = dct_t_l3f11faam_production.objects.filter(pjcode=pjCode, owner__icontains=keyWord,
                                                               activetime__range=(dayTimeStart, dayTimeEnd))
            for line in result:
                #                 activeTime = line.activetime
                #                 if activeTime==None:
        ...
        nameList = []
        for line in result:
            temp = {'employee': line.employee, 'unitprice': line.unitprice, 'position': line.position,
                    'zone': line.zone}
            nameList.append(temp)
        typeList = self.__dft_get_product_type(pjCode)
        workingDay = {}
        ...
        for i in range(len(workBuf)):
            ...
            if (employee in workingDay.keys()) and (employee in totalWorkTime.keys()) and (
                    employee in totalTimeSalary.keys()):
                if workTime != 0:
                    workingDay[employee] = workingDay[employee] + 1
                    totalWorkTime[employee] = totalWorkTime[employee] + workTime
        ...
        for i in range(len(nameList)):
            ...
            if (employee in workingDay.keys()) and (employee in totalWorkTime.keys()):
                if employee in packageSum.keys():
                    tempPackageSum = packageSum[employee]
                else:
                    tempPackageSum = 0
                if employee in weightSum.keys():
                    tempWeightSum = weightSum[employee]
                else:
                    tempWeightSum = 0
                if employee in numSum.keys():
                    tempNumSum = numSum[employee]
                else:
                    tempNumSum = 0
                if employee in standardNumList.keys():
                    totalStandardNum = standardNumList[employee]
                else:
                    totalStandardNum = 0
    ...
    def dft_dbi_consumables_buy(self, inputData):
        ...
        dct_t_l3f11faam_buy_consumables.objects.create(supplier=supplier, contype=contype, amount=number,
                                                       unitprice=unit_price, totalprice=total_price,
                                                       createtime=storage_time, datatype=datype)
    ...
    def dft_dbi_get_print(self, inputData):
        ...
        if result.exists():
            for line in result:
                value = [line.sid, line.contype, line.supplier, line.datatype, line.amount, str(line.unitprice),
                         str(line.totalprice), str(line.createtime)]
    ...
    def dft_dbi_consumables_history_table(self, inputData):
        ...
        if keyWord == "":
            if key == "":
                result = dct_t_l3f11faam_buy_consumables.objects.filter(createtime__gte=timeStart,
                                                                        createtime__lte=timeEnd)
            else:
                result = dct_t_l3f11faam_buy_consumables.objects.filter(contype=key, createtime__gte=timeStart,
                                                                        createtime__lte=timeEnd)
        else:
            if key == "":
                result = dct_t_l3f11faam_buy_consumables.objects.filter(createtime__gte=timeStart,
                                                                        createtime__lte=timeEnd,
                                                                        supplier__icontains=keyWord)
            else:
                result = dct_t_l3f11faam_buy_consumables.objects.filter(contype=key, createtime__gte=timeStart,
                                                                        createtime__lte=timeEnd,
                                                                        supplier__icontains=keyWord)
    ...
    def dft_dbi_get_consumbales_purchase(self, inputData):
        ...
        for line in result:
            ...
            if line.contype == "纸箱":
                consumables_value.append('1')
            elif line.contype == "保鲜袋":
                consumables_value.append('2')
            elif line.contype == "胶带":
                consumables_value.append('3')
            elif line.contype == "标签":
                consumables_value.append('4')
            elif line.contype == "托盘":
                consumables_value.append('5')
            elif line.contype == "垫片":
                consumables_value.append('6')
            elif line.contype == "网套":
                consumables_value.append('7')
            elif line.contype == "打包带":
                consumables_value.append('8')
            else:
                break
    ...
    def dft_dbi_consumables_purchase_mod(self, inputData):
        ...
        if type == "":
            ...
            dct_t_l3f11faam_buy_consumables.objects.filter(sid=sid).update(supplier=vendor, contype=type, amount=number,
                                                                           unitprice=unit, totalprice=total,
                                                                           createtime=storage_time)
            return True
    ...
    def dft_dbi_product_stock_new(self, inputData):
        ...
        if result.exists():
            dct_t_l3f11faam_product_stock_sheet.objects.filter(stockname=name, stockaddress=address,
                                                               createtime=data_now)
        else:
            productweight = 0
            productnum = 0
            number = 0
            dct_t_l3f11faam_product_stock_sheet.objects.create(stockname=name, stockaddress=address,
                                                               productweight=productweight, productnum=productnum,
                                                               number=number, createtime=data_now, updatetime=data_now)
    ...
    def dft_dbi_get_product_weight_and_size(self):
        ...
        for line in result:
            ...
            if line.applegrade == 'A':
                grade = '特级'
            elif line.applegrade == '1':
                grade = '一级'
            elif line.applegrade == '2':
                grade = '二级'
            elif line.applegrade == '3':
                grade = '三级'
            elif line.applegrade == 'S':
                grade = '混合'
    ...
    def dft_dbi_product_stock_transfer(self, inputData):
        ...
        for line in result:
            name = line.stockname
        result = dct_t_l3f11faam_product_stock_sheet.objects.filter(stockname=name, productweight=weight,
                                                                    puoductsize=size, productnum=28)
        for line in result:
        ...
        if number > productNumber:
            ...
            out_stock = dct_t_l3f11faam_product_stock_sheet.objects.get(stockname=name, productweight=weight,
                                                                        puoductsize=size, productnum=28)
            ...
            into_stock = dct_t_l3f11faam_product_stock_sheet.objects.filter(sid=target, productweight=weight,
                                                                            puoductsize=size, productnum=productNum)
            if into_stock.exists():
                update_stock = dct_t_l3f11faam_product_stock_sheet.objects.get(sid=target, productweight=weight,
                                                                               puoductsize=size, productnum=productNum)
                ...
                dct_t_l3f11faam_product_history.objects.create(stockname=name, productweight=weight, puoductsize=size,
                                                               productnum=productNum,
                                                               number=number, containerID='---', platenumber='---',
                                                               drivername='---', driverphone='---',
                                                               receivingunit=intoStockInfo.stockname,
                                                               logisticsunit='---',
                                                               message=note)
            else:
                if intoStockInfo:
                    ...
                    intoStockInfo.save(
                        update_fields=['productweight', 'puoductsize', 'productnum', 'number', 'updatetime', 'message'])
                    ...
                    dct_t_l3f11faam_product_stock_sheet.objects.create(message=note, stockname=intoStockInfo.stockname,
                                                                       createtime=intoStockInfo.createtime,
                                                                       stockaddress=intoStockInfo.stockaddress,
                                                                       productweight=weight, puoductsize=size,
                                                                       productnum=productNum, updatetime=date_now,
                                                                       number=number)
                    dct_t_l3f11faam_product_history.objects.create(stockname=name, productweight=weight,
                                                                   puoductsize=size, productnum=productNum,
                                                                   number=number, containerID='---', platenumber='---',
    ...
    def dft_dbi_product_stock_history(self, inputData):
        ...
        if stockName == "":
            if keyWord == "":
                ...
                result = dct_t_l3f11faam_product_history.objects.filter(outtime__gte=timeStart, outtime__lte=timeEnd,
                                                                        drivername__icontains=keyWord)
        else:
            if keyWord == "":
                result = dct_t_l3f11faam_product_history.objects.filter(stockname=stockName, outtime__gte=timeStart,
                                                                        outtime__lte=timeEnd)
            else:
                result = dct_t_l3f11faam_product_history.objects.filter(stockname=stockName,
                                                                        drivername__icontains=keyWord,
                                                                        outtime__gte=timeStart, outtime__lte=timeEnd)
        i = 1
        for line in result:
            if line.puoductsize == "A":
                size = '特级'
    ...
    def dft_dbi_get_product_stock_history_detail(self, inputData):
        ...
        for line in result:
            ...
            result = {'storageID': stockID, 'weight': weight, 'size': size, 'number': number, 'container': container,
                      'trunk': trunk, 'driver': driver, 'mobile': mobile, 'target': target, 'logistics': logistics}
        return result
    ...
    def dft_dbi_material_stock_new(self, inputData):
        ...
        if result.exists():
            dct_t_l3f11faam_material_stock_table.objects.filter(stockname=name).update(stockaddress=address,
                                                                                       isself=Tableisself)
        else:
            dct_t_l3f11faam_material_stock_table.objects.create(stockname=name, stockaddress=address, stockleader="-",
                                                                isself=Tableisself, bucketnum=0, totalprice=0)
            ...
            for line in result:
                if bucket > line.bucketnum:
                    ...
                    dct_t_l3f11faam_material_history.objects.create(stockid=ID, stockname=line.stockname,
                                                                    ...
                                                                    mobile=mobile, trunk=trunk, target=target,
    ...
    def dft_dbi_material_stock_history(self, inputData):
        ...
        if (ID == 'all'):
            if keyWord == "":
                ...
                result = dct_t_l3f11faam_material_history.objects.filter(time__gte=timeStart, time__lte=timeEnd,
                                                                         charge__icontains=keyWord)
        else:
            if keyWord == "":
                result = dct_t_l3f11faam_material_history.objects.filter(time__gte=timeStart, time__lte=timeEnd,
                                                                         stockid=ID)
            else:
                result = dct_t_l3f11faam_material_history.objects.filter(stockid=ID, time__gte=timeStart,
                                                                         time__lte=timeEnd, charge__icontains=keyWord)
        i = 1
        for line in result:
            empty = []
            name = line.stockname
    ...
    def dft_dbi_get_material_stock_history_deatil(self, inputData):
        ...
        for line in result:
            if line.mode:
                ...
                materialMode = '1'
            if line.into:
                table = {'type': '0', 'storageID': line.stockid, 'materialMode': materialMode, 'bucket': line.bucketnum,
                         'price': line.price, 'buyer': line.charge, 'vendor': line.vendor, 'mobile': line.mobile}
            else:
        ...
        for line in result:
            ...
            if result.exists():
                ...
                if new_number + old_number - number >= 0:
                    ...
                    line.save(update_fields=['number', 'containerID', 'platenumber', 'drivername', 'driverphone',
                                             'receivingunit', 'logisticsunit'])
                else:
                    return False
            else:
                return False
    ...
    def dft_dbi_faam_qrcode_audit(self, inputData):
        ...
        if keyWord=="":
            ...
            result = dct_t_l3f11faam_production.objects.filter(pjcode=pjCode, activetime__range=(timeStartStr, timeEndStr),activeman__icontains=keyWord)
        i = 1
        a = {'TimeStart': '2018-11-14', 'TimeEnd': '2018-11-14', 'KeyWord': '', 'uid': 'UID0000001'}
        if result.exists():
            for line in result:
                owner=line.activeman
                type=line.typecode
                if owner in data.keys():
                    if type in data[owner].keys():
                        data[owner][type][0]=data[owner][type][0]+1
                    else:
                        data[owner][type] = []
                        data[owner][type].append(1)
                else:
                    data[owner]={}
                    if type in data[owner].keys():
                        data[owner][type][0]=data[owner][type][0]+1
        ...
        for key,value in data.items():
            for key_type in data[key].keys():
                history = []
                result_type=dct_t_l3f11faam_type_sheet.objects.filter(typecode=key_type)
                if result_type.exists():
                    type_number=result_type[0].applenum
                    type_weight=result_type[0].appleweight
                    totalNumber=data[key][key_type][0]*type_number
                    totalWeight=data[key][key_type][0]*type_weight
                else:
                    totalNumber="未知规格代码"
                    totalWeight="未知规格代码"
                history.append(i)
                ...
                history.append(data[key][key_type][0])
                history.append(str(totalNumber))
    ...
    def dft_dbi_faam_qrcode_kq_process(self, inputData):
        ...
        if membersheet.exists():
            for line in membersheet:
                ...
                if employee != "":
                    unitPrice = line.unitprice
                    dailysheet = dct_t_l3f11faam_daily_sheet.objects.filter(pjcode=scanCode, employee=employee,
                                                                            workday=workDay)
                    if dailysheet.exists():
                        for line_daily in dailysheet:
                            ...
                            dct_t_l3f11faam_daily_sheet.objects.filter(pjcode=scanCode, employee=employee,
                                                                       workday=workDay).update(
                                daystandardnum=dayStandardNum, leavetime=currentTime, worktime=WorkTime,
                                unitprice=unitPrice, laterflag=lateWorkFlag, earlyflag=earlyLeaveFlag)
                            resp = {'employee': employee, 'message': '考勤成功'}
                    else:
                        dct_t_l3f11faam_daily_sheet.objects.create(pjcode=scanCode, employee=employee, workday=workDay,
                                                                   arrivetime=currentTime, offwork=0)
                        resp = {'employee': employee, 'message': '考勤成功'}
        ...
        if result.exists():
            ...
            if lastNum + applyNum < 9999:
                ...
                for i in range(start, end):
                    ...
                    qrcode = labelBaseInfo + digNum
                    qrcode_list.append(
                        dct_t_l3f11faam_batch_scan(qrcode=qrcode, owner=userTabTL, typecode=userTabTR, applyweek=week,
                                                   applydate=currentTime, packagesum=Number))
            ...
            for key, value in employee_type.items():
                ...
                for key_value, value_value in employee_type[employee].items():
                    if key_value == 'pjcode':
                        ...
                        type = key_value
                        package = employee_type[employee][key_value][0]
                        weightnum = float(employee_type[employee][key_value][1])
                        number = employee_type[employee][key_value][2]
                        result_sta = dct_t_l3f11faam_production_sta.objects.filter(employee=employee, pjcode=pjcode,
                                                                                   activedate=date, typecode=type)
                        if result_sta.exists():
                            result_sta.update(packagesum=package, weightsum=weightnum, numsum=number)
                        else:
                            sta_list.append(dct_t_l3f11faam_production_sta(pjcode=pjcode, employee=employee, typecode=type,
                                                                           activedate=date, packagesum=package,
                                                                           weightsum=weightnum, numsum=number))
